...
export async function searchReference(query: string) {
            ...
            requests: [
                {
                    ...
                    params: `&query=${encodeURIComponent(query)}&maxValuesPerFacet=50`
                }
            ]
        }),
        method: 'POST',
    ...
    const englishResults: Array<SearchResult> = result.results[0].hits.filter((i: any) => i.language === 'en').slice(0, 10);
